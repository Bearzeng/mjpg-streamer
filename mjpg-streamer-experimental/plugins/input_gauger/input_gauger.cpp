...
{
{CMD_INIT_TARGET, V4L2_CTRL_TYPE_BUTTON, "初始化",  1, 0, 0, 0, 0, 0},
0,
NULL,
0,
IN_CMD_GENERIC
...
{
{CMD_SNAPSHUT, V4L2_CTRL_TYPE_BUTTON, "截图", 1, 0, 0, 0, 0, 0},
0,
NULL,
0,
IN_CMD_GENERIC
...
{
{CMD_CAL_ONCE, V4L2_CTRL_TYPE_BUTTON, "计算1次", 1, 0, 0, 0, 0, 0},
0,
NULL,
0,
IN_CMD_GENERIC
...
CGauger* get_gauger_instance(int height, int width)
{
    TEMPPARAM tempParam;
...
void *worker_thread(void *arg)
{
    ...
    Mat src; // 单色原始图像，用于计算
    Mat dst = Mat(540, 720, CV_8UC3); // 将src->彩色，用于在上面画彩色标记
    vector<uchar> jpeg_buffer;
    Scalar color = Scalar(0, 0, 255);
    
    CGauger *gauger = get_gauger_instance(540, 720);
    ...
    while (!pglobal->stop) {
        ...
        g_cmd = CMD_UNKNOWN; // reset the cmd

        //cv::imwrite("/tmp/"+to_string(a)+".bmp", src);
...
int input_cmd(int plugin, unsigned int control_id, unsigned int group, int value, char *value_string)
{
    /*SLOGI*/ printf("command (%d, value: %ld) for group %d triggered for plugin instance #%02d\n", control_id, value, group, plugin);
#if 1
    switch(control_id){
    ...
    case CMD_STOP:
        ...
        break;
    case CMD_RESUME:
        return 0;//start_calculating(true);
        break;
    case CMD_CAL_ONCE:
        //calculate_once(0);
        break;
